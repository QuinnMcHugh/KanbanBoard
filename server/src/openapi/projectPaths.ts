...
registry.registerPath({
    ...
    description:
        "Partial update. Any authenticated user may rename or reassign any project — there is no per-owner access restriction in this API.",
    security: [{ bearerAuth: [] }],
    request: {
...
registry.registerPath({
    ...
    description:
        "Cascades — also deletes every task under this project (and each task's label associations).",
    security: [{ bearerAuth: [] }],
    request: { params: projectIdParamSchema },
    responses: {
        200: {
