...
registry.registerPath({
    ...
    description:
        "404s if the task exists but belongs to a different project than :projectId.",
    security: [{ bearerAuth: [] }],
    ...
    responses: {
        ...
        404: notFoundResponse(
            "Project not found, or task not found, or task belongs to a different project.",
        ),
    },
})
...
registry.registerPath({
    ...
    description:
        "project_id comes from the URL. Creating a task with an invalid assigned_to_user_id or labelIds rolls back the whole operation — no task is created.",
    security: [{ bearerAuth: [] }],
    request: {
    ...
    responses: {
        ...
        ...authFailureResponses,
        404: notFoundResponse(
            "Project not found, or assigned_to_user_id does not reference an existing user.",
        ),
    },
})
...
registry.registerPath({
    ...
    description:
        "Partial update. Omitting labelIds leaves existing labels untouched; sending [] clears them; sending ids replaces the whole set. project_id in the body (not the URL) is what actually moves a task between projects.",
    security: [{ bearerAuth: [] }],
    request: {
    ...
    responses: {
        ...
        ...authFailureResponses,
        404: notFoundResponse(
            "Project not found, task not found, or task belongs to a different project.",
        ),
    },
})
