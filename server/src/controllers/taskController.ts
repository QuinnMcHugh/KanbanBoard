...
async function assertUserIdExists(
    ...
    if (assigned_to_user_id) {
        ...
        if (!user) {
            throw new NotFoundError(
                "The user for this task could not be found.",
            )
        }
    }
}

async function assertLabelIdsExist(
    queryBuilder: Knex | Knex.Transaction = db,
    ...
    if (labelIds?.length) {
        ...
        if (labels.length !== labelIds.length) {
            const existingLabelsSet = new Set<number>(
                labels.map(({ id }) => id),
            )
            ...
            throw new BadRequestError(
                `Labels with these id's not found: ${[...missingLabelsSet].join(", ")}`,
            )
        }
    }
}

async function createTaskLabelsRecords(
    queryBuilder: Knex | Knex.Transaction = db,
    labelIds: number[] | undefined,
    newTaskId: number,
) {
    ...
    if (labelIds?.length) {
        const newTaskLabels = labelIds.map((label_id: number) => ({
            task_id: newTaskId,
            label_id,
        }))
        await queryBuilder("task_labels").insert(newTaskLabels)
    }
}
...
export async function getTasks(
    ...
    const rows = await tasksWithLabels().where("tasks.project_id", project_id)
...
export async function getTask(
    req: Request<{ id: string; projectId: string }>,
    res: Response,
): Promise<void> {
...
export async function createTask(
    req: Request<{ projectId: string }, unknown, CreateTaskInput>,
    res: Response,
): Promise<void> {
    await assertProjectExists(req)

    const projectId = Number(req.params.projectId)
    const { name, description, status, assigned_to_user_id, labelIds } =
        req.body
...
export async function updateTask(
    req: Request<{ id: string; projectId: string }, unknown, UpdateTaskInput>,
    res: Response,
): Promise<void> {
    await assertProjectExists(req)
    ...
    const task = await db.transaction(async (trx) => {
        ...
        if (labelIds) {
            ...
            await trx("task_labels").where({ task_id: taskId }).delete()
