...
function hasNumericStatus(
    err: unknown,
): err is { status: number; message?: unknown } {
    return (
        typeof err === "object" &&
        err !== null &&
        "status" in err &&
...
export const errorHandler: ErrorRequestHandler = (
    ...
    if (hasNumericStatus(err) && err.status < 500) {
        const message =
            typeof err.message === "string" ? err.message : "Malformed request."
        res.status(err.status).json({ error: message })
        return
    }

    if (isSqliteConstraintError(err)) {
        const uniqueMatch = err.message.match(
            /UNIQUE constraint failed: \w+\.(\w+)/,
        )
        if (uniqueMatch) {
            res.status(409).json({
                error: `A record with this ${uniqueMatch[1]} already exists.`,
            })
            return
        }
