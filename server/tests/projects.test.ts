...
describe("GET /api/projects", () => {
    ...
    it("lists all projects with joined owner info", async () => {
        ...
        const websiteRedesign = res.body.projects.find((p: any) => p.name === "Website Redesign");
        expect(websiteRedesign).toMatchObject({ owner: "alice_admin", owner_email: "alice@example.com" });
    });
});
...
describe("GET /api/projects/:id", () => {
    it("returns a single project", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.project).toMatchObject({ id: 1, name: "Website Redesign", owner: "alice_admin" });
    });
...
describe("POST /api/projects", () => {
    it("creates a project scoped to an explicit owner_id", async () => {
        ...
        expect(res.body.project).toMatchObject({ name: "New Project", owner: "bob_builder" });
    });

    it("400s when name is missing", async () => {
        const res = await request(app).post("/api/projects").set(authHeader()).send({ owner_id: 1 });
        expect(res.status).toBe(400);
    });

    it("400s when owner_id is missing", async () => {
        const res = await request(app).post("/api/projects").set(authHeader()).send({ name: "No Owner" });
        expect(res.status).toBe(400);
    });
...
describe("PATCH /api/projects/:id", () => {
    it("updates only the provided field, leaving the rest untouched", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.project).toMatchObject({ id: 1, name: "Renamed Project", owner: "alice_admin" });
    });

    it("reassigns the owner", async () => {
        const res = await request(app).patch("/api/projects/1").set(authHeader()).send({ owner_id: 2 });
    ...
    it("400s when no fields are provided", async () => {
        const res = await request(app).patch("/api/projects/1").set(authHeader()).send({});
        expect(res.status).toBe(400);
    });

    it("404s for a nonexistent id", async () => {
        const res = await request(app).patch("/api/projects/9999").set(authHeader()).send({ name: "Ghost" });
        expect(res.status).toBe(404);
    });

    it("400s when reassigned to a nonexistent owner", async () => {
        const res = await request(app).patch("/api/projects/1").set(authHeader()).send({ owner_id: 9999 });
        expect(res.status).toBe(400);
    });
});
...
describe("DELETE /api/projects/:id", () => {
    it("deletes a project and returns its joined data", async () => {
        const res = await request(app).delete("/api/projects/3").set(authHeader());

        expect(res.status).toBe(200);
        expect(res.body.project).toMatchObject({ id: 3, name: "Internal Tools" });
    });
