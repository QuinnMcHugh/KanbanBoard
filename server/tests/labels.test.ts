...
describe("GET /api/labels/:id", () => {
    it("returns a single label", async () => {
        ...
        expect(res.status).toBe(200);
        expect(res.body.label).toMatchObject({ id: 1, name: "Frontend", color: "#3b82f6" });
    });
...
describe("POST /api/labels", () => {
    it("creates a label and returns the full row from a single query", async () => {
        ...
        expect(res.body.label).toMatchObject({ name: "Documentation", color: "#00ffff" });
        expect(res.body.label.id).toEqual(expect.any(Number));
    });

    it("400s when name is missing", async () => {
        const res = await request(app).post("/api/labels").set(authHeader()).send({ color: "#000000" });
        expect(res.status).toBe(400);
    });

    it("400s when color is missing", async () => {
        const res = await request(app).post("/api/labels").set(authHeader()).send({ name: "No Color" });
        expect(res.status).toBe(400);
    });
...
describe("PATCH /api/labels/:id", () => {
    it("updates only the provided field", async () => {
        const res = await request(app).patch("/api/labels/1").set(authHeader()).send({ color: "#ffffff" });

        expect(res.status).toBe(200);
        expect(res.body.label).toMatchObject({ id: 1, name: "Frontend", color: "#ffffff" });
    });

    it("400s when no fields are provided", async () => {
        const res = await request(app).patch("/api/labels/1").set(authHeader()).send({});
        expect(res.status).toBe(400);
    });

    it("404s for a nonexistent id", async () => {
        const res = await request(app).patch("/api/labels/9999").set(authHeader()).send({ color: "#ffffff" });
        expect(res.status).toBe(404);
    });
...
describe("DELETE /api/labels/:id", () => {
    ...
    it("404s for a nonexistent id", async () => {
        const res = await request(app).delete("/api/labels/9999").set(authHeader());
        expect(res.status).toBe(404);
    });
