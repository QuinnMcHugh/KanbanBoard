...
} from "../src/schemas/labelSchemas"
import {
    taskResponseSchema,
    taskListResponseSchema,
} from "../src/schemas/taskSchemas"
...
describe("response schemas match reality", () => {
    ...
    it("projects: list and single", async () => {
        const listRes = await request(app)
            ...
            .set(authHeader())
        expect(() =>
            projectListResponseSchema.parse(listRes.body),
        ).not.toThrow()
    ...
    it("tasks: list, single (with labels), and one with no labels", async () => {
        ...
        const withLabels = await request(app)
            .get("/api/projects/1/tasks/4")
            .set(authHeader())
        expect(() => taskResponseSchema.parse(withLabels.body)).not.toThrow()

        const noLabels = await request(app)
